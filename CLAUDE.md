...
`importar.html` + `js/importar.js` is the site-facing page: paste a playlist link, it calls the Worker in batches, shows a progress bar across the whole playlist, displays the playlist name, and offers the accumulated result as a download (filename `uque-import-<slug-do-nome>-<data>.json`) importable via the existing backup-import feature. Styled to match the rest of the site (same header/logo/`style.css`). `WORKER_URL` in `js/importar.js` points to the deployed Worker above.
